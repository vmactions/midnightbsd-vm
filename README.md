...
>
> Just describe what you want in plain language, e.g. *"run my tests on MidnightBSD"* or *"check that my project builds on MidnightBSD aarch64"*, and the agent generates a correct, ready-to-commit `test.yml`. It will:
>
...
Use this action to run your CI in MidnightBSD.

The github workflow only supports Ubuntu, Windows and MacOS. But what if you need to use MidnightBSD?
...
jobs:
  test:
    ...
    name: A job to run test in MidnightBSD
    env:
...
It uses [the MidnightBSD 4.0.4](conf/default.release.conf) by default, you can use `release` option to use another version of MidnightBSD:
...
We use Qemu to run the MidnightBSD VM.
